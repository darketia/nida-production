    ...
    <div>
      <g:form action="index" class="form-horizontal skipWaiting">
        ...
        <div class="form-group">
          <label for="code" class="col-sm-2 control-label"><g:message code="vendor.code.label" default="Code"/></label>
          <div class="col-sm-4"><g:textField class="form-control" id="code" name="code" value="${params.code}"/></div>
        </div>

        <div class="form-group">
          <label for="name" class="col-sm-2 control-label"><g:message code="vendor.name.label" default="Name"/></label>
...
<div class="row">
  <div class="table-responsive">
    <table class="table">
      <thead>
      <tr>
        <g:sortableColumn class="text-center" property="code" title="${message(code: 'vendor.code.label', default: 'Code')}" params="${queryParams}"/>
        <g:sortableColumn class="text-center" property="name" title="${message(code: 'vendor.name.label', default: 'Name')}" params="${queryParams}"/>
        <g:sortableColumn class="text-center" property="address" title="${message(code: 'vendor.address.label', default: 'address')}" params="${queryParams}"/>
      ...
      <g:each in="${vendorInstanceList}" status="i" var="vendorInstance">
        <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
          ...
          <td><g:link action="show" id="${vendorInstance.id}">${fieldValue(bean: vendorInstance, field: "code")}</g:link></td>
          
          <td>${fieldValue(bean: vendorInstance, field: "name")}</td>
          <td>${fieldValue(bean: vendorInstance, field: "address")}</td>

          <td class="center">
            <g:link class="btn btn-info" action="show" id="${vendorInstance.id}"><g:message code="default.button.show.label" default="Show"/></g:link>
            <g:link class="btn btn-primary" action="edit" id="${vendorInstance.id}"><g:message code="default.button.edit.label" default="Edit"/></g:link>
          </td>
        </tr>
      </g:each>
      </tbody>
    ...
    <div class="text-center">
      <g:paginate total="${vendorInstanceCount ?: 0}" params="${queryParams}"/>
    </div>
  </div>
</div>
</body>
